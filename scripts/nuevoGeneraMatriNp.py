...
def pantallaMenu():
    ...
    print("4. Modificar porcentualmente los valores de una matriz")
    print("0. Salir\n")
...
while True:
    ...
    if opcion.isnumeric():
        ...
        if opcion == 1: # Sumar matrices
            ...
            print("\n¿Qué matriz deseas transponer?\n")
            print(f"1. Matriz {nomMatrizA}")
            print(f"2. Matriz {nomMatrizB}")
            op = input("\nSelecciona el nro. de la opción de la matriz a transponer: ")
            if op == '1':
                matriz = matrizA.T
            ...
            print("\n¿Qué matriz deseas aumentar o disminuir?\n")
            print(f"1. Matriz {nomMatrizA}")
            print(f"2. Matriz {nomMatrizB}")
            op = input("\nSelecciona el nro. de la opción de la matriz a modificar: ")
            if op == '1':
                razon = float(input(f"Indica la razón (porcentaje decimal) en la que deseas modificar la matriz {nomMatrizA}: "))
                print(f"\nLos nuevos valores de la matriz {nomMatrizA} son:")
                matrizA = matrizA.dot(razon)
                print(matrizA)
                pausa()
            elif op == '2':
                razon = float(input(f"Indica la razón (porcentaje decimal) en la que deseas modificar la matriz {nomMatrizB}: "))
                print(f"\nLos nuevos valores de la matriz {nomMatrizB} son:")
                matrizB = matrizB.dot(razon)
                print(matrizB)
                pausa()
            else:
                error("Opción no válida !!! (╯‵□′)╯︵┻━┻")
